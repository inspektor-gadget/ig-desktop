            ...
            if (this._selectionHandler !== undefined || tooltipMetrics.length > 0) {
                ...
                if (tooltipMetrics.length > 0) {
                    ...
                    const mouseMoved = ({offsetX: x, offsetY: y}) => {
                        ...
                        const row = date - this._data[index - 1][this._xField] > this._data[index][this._xField] - date
                            ? this._data[index]
                            : this._data[index - 1];
...
{#if data.length}
	<div class="sticky left-0" use:initialize use:resizeObserver={element => width = element.clientWidth} bind:this={svg}></div>
{/if}
