...
<div class="flex flex-col gap-1 {className}">
	{#if label}
		<label for={inputId} class="text-sm font-medium text-ig-text">
			{label}
			{#if required}
	...
	{#if description}
		<p class="text-sm text-ig-text-muted">{description}</p>
	{/if}

	<div class="relative">
		<!-- Input container with tags for multi-select -->
		<div
			class="flex min-h-[42px] flex-wrap items-center gap-2 rounded-ig-md border bg-ig-surface-raised px-3 py-2 transition-colors {error
				? 'border-ig-error'
				: 'border-ig-border-strong focus-within:border-ig-primary focus-within:ring-2 focus-within:ring-ig-primary-muted'} {disabled
		...
		>
					...
					>
						...
						{#if !disabled}
							<button
								...
								class="text-ig-text-muted transition-colors hover:text-ig-text"
			...
			<input
				...
				class="min-w-[100px] flex-1 border-none bg-transparent text-ig-text placeholder-ig-text-muted outline-none"
				autocomplete="off"
			/>
	...
	{#if error}
		<p class="text-sm text-ig-error">{error}</p>
	{/if}
</div>
