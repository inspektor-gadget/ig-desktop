	...
	{#each gadgetInfo.params as param}
		<li class="border-b border-b-ig-border py-2 last:border-b-0">
			...
			{#if param.description}
				<div
					class="mb-2 max-h-16 overflow-hidden text-xs overflow-ellipsis whitespace-pre-wrap text-ig-text-muted"
				>
					{param.description}
				</div>
			{/if}
			<div>
				<input
					class="w-full rounded-ig-sm bg-ig-surface-raised p-1"
					type="text"
