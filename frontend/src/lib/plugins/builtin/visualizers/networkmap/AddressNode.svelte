<script lang="ts">
	...
	const labels = $derived(
		data.labels?.filter((label) => label.field !== 'namespace') ?? [
			{ field: 'addr', value: data.addr }
		]
	);
	const label = $derived(labels[0]?.value ?? data.label);
		...
		<div class="px-1 py-1">
			{#each handles as handle (handle.id)}
				...
				>
					...
					{#if handle.type === 'target' || handle.type === 'both'}
					...
					{/if}
					{#if handle.type === 'source' || handle.type === 'both'}
