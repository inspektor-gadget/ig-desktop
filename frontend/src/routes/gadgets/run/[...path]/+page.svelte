...
<div class="flex grow flex-row gap-6 overflow-auto p-6">
	<div class="flex grow flex-col gap-6">
		{#if !gadgetInfo}
			{#if error}
				<div class="rounded bg-red-950/50 p-4 text-red-400">{error}</div>
			{:else if !environmentID}
				<div class="rounded bg-gray-900/50 p-4 text-gray-400">
					Please select an environment from the dropdown above to continue.
				</div>
			{:else}
				<div class="p-4">Loading gadget information...</div>
			{/if}
		{:else}
