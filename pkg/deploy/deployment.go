	...
	return api.Params{
		{
			...
			DefaultValue: "ghcr.io/inspektor-gadget/inspektor-gadget:latest", // TODO
			IsMandatory:  true,
		},
		{
		...
		},
		{
			...
			Tags:         []string{ParamGroupSecurity, "advanced", "multiline"},
		},
		{
		...
		},
		{
			...
			DefaultValue: resources.InspektorGadgetPublicKey,
			Tags:         []string{ParamGroupSecurity, "multiline"},
		},
		{
...
func (d *Deployment) ValidateParamValues(pv api.ParamValues) error {
	nv, err := pv.ValidateWithDefaults(d.Params())
	if err != nil {
		return err
	}
