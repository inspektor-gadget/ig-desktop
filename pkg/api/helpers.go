...
var typeHintValidators = map[string]ParamValidator{
...
// ValidateWithDefaults will validate constraints and set default values; it will return a new validated instance of ParamValues
// only containing values for the params given.
func (pv ParamValues) ValidateWithDefaults(params Params) (ParamValues, error) {
	var err error
	...
	for _, param := range params {
		...
		if validator, ok := typeHintValidators[param.TypeHint]; ok {
			tmpErr := validator(v)
			if tmpErr != nil {
				err = errors.Join(err, fmt.Errorf("validating %q: %w", param.Key, tmpErr))
			}
		}
	}
	if err != nil {
